...
export const useSingleCallMethodMulti = <
  C extends Contract,
...
>(
  ...
  useSyncEffect(() => {
    if (!contract || !chainId || !inputs || disabled) return;

    const stale: Stale = { value: false };

    const fetch = async (inp: P): Promise<Result | Default> =>
      await retry((bail) => {
        if (stale.value) {
          bail(new Error('stale'));
          return;
        }
        return contract.callStatic[methodName as string](inp);
      }, retryOptions);

    const fetchs = inputs?.map((i) => {
      return () => fetch(i);
    });
  ...
  return useMemo(
    ...
    [result, freshLoading, error, chainId],
  );
};
...
const fetchStateHandler = async <T>(
  ...
  stale: Stale,
  // fetch: () => Promise<T>,
  fetchs: Array<() => Promise<T>>,
  errorHandler?: (error: any) => void,
) => {
  try {
    setLoading(true);
    const awated = await Promise.all(
      fetchs.map(async (fetch) => {
        const fetchResult: T = await fetch();
        return fetchResult;
      }),
    );
    const fetchResults = awated as T;
    if (JSON.stringify(fetchResults) !== JSON.stringify(prevResult) && !stale.value) {
